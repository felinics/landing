...
export function seedBotRoutes(): Map<string, any> {
  ...
  for(const bot of bots) {
    ...
    put('/token-usage',{chat:days,discuss:days.map(d=>({...d,input_tokens:2400,output_tokens:680,cache_read_tokens:1200,reasoning_tokens:100})),schedule:days.map(d=>({...d,input_tokens:1800,output_tokens:540,cache_read_tokens:800,reasoning_tokens:80})),acp_agent:[],by_model:[{model_id:'model-1',model_name:'Claude Sonnet 4',model_slug:'claude-sonnet-4',provider_name:'Anthropic',input_tokens:325000,output_tokens:89000,cache_read_tokens:183000,reasoning_tokens:13000}]})
    put('/token-usage/records',{items:days.slice(-12).reverse().map((d,i)=>({...d,id:`usage-${i}`,created_at:`${d.day}T08:00:00Z`,session_type:'chat',session_id:'session-welcome',model_id:'model-1',model_name:'Claude Sonnet 4',provider_name:'Anthropic'})),total:12})
