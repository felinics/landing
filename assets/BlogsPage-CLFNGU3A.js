import{$ as e,A as t,C as n,Dt as r,O as i,S as a,T as o,U as s,V as c,Z as l,a as u,i as d,k as f,lt as p,r as m,rt as h,t as g,v as _,w as v,x as y}from"./createLucideIcon-CVuxzMVS.js";import{t as ee}from"./arrow-left-CZ-a0lG8.js";import{t as b}from"./arrow-right-hXxXd9HO.js";import{t as x}from"./calendar-days-mdGmnSk2.js";import{t as S}from"./clock-3-CdPC6wqV.js";import{c as te,i as C,l as w,s as T,t as E}from"./index-CGlxGx59.js";var D=g(`file-text`,[[`path`,{d:`M6 22a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.704.706l3.588 3.588A2.4 2.4 0 0 1 20 8v12a2 2 0 0 1-2 2z`,key:`1oefj6`}],[`path`,{d:`M14 2v5a1 1 0 0 0 1 1h5`,key:`wfsgrz`}],[`path`,{d:`M10 9H8`,key:`b1mrlr`}],[`path`,{d:`M16 13H8`,key:`t4e002`}],[`path`,{d:`M16 17H8`,key:`z1uh3a`}]]),O=g(`user-round`,[[`circle`,{cx:`12`,cy:`8`,r:`5`,key:`1hypcn`}],[`path`,{d:`M20 21a8 8 0 0 0-16 0`,key:`rfgkzh`}]]),k=Object.assign({"../content/blogs/en/2026-02-16.md":`---
...
`,"../content/blogs/en/cloud-computer-for-every-agent.md":`---
title: Introducing Memoh: A Cloud Computer for Every Agent
author: Yanbo
date: 2026-09-15
---
...
[Start with Memoh Cloud](https://app.memoh.net) · [Download Desktop](https://memoh.ai/download) · [Read the docs](https://docs.memoh.ai) · [GitHub](https://github.com/felinics/Memoh)
`,"../content/blogs/en/introduction-to-memoh.md":`---
title: Introduction to Memoh - The Case for an Always-On, Containerized Home Agent
author: Team Memoh
date: 2026-02-16
---
...
Furthermore, we will continue to operate and permanently open-source Memoh, making it a product with long impact.`,"../content/blogs/zh/2026-05-02.md":`---
...
`,"../content/blogs/zh/cloud-computer-for-every-agent.md":`---
title: 介绍 Memoh：给每个 Agent 一台云电脑
author: Yanbo
date: 2026-09-15
---
...
[开始使用 Memoh Cloud](https://app.memoh.net) · [下载桌面版](https://memoh.ai/download) · [阅读文档](https://docs.memoh.ai) · [GitHub](https://github.com/felinics/Memoh)
`,"../content/blogs/zh/discuss-mode.md":`---
title: 介绍Memoh的Discuss模式 - 我们如何实现一个更主动的群聊机器人
author: Acbox
date: 2026-05-02
---
...
Discuss 模式的核心理念很简单：不要把 bot 当工具，把它当群成员。让它听得到所有对话，让它自己决定什么时候说话。剩下的，交给 LLM 的推理能力。
`,"../content/blogs/zh/workspace-desktop.md":`---
title: 提供桌面，我们可以做到更多 - 介绍Memoh的桌面电脑功能
author: Ran Chen
date: 2026-05-15
---
...
给 bot 一个 workspace 桌面以后，它可以打开页面、看状态、点按钮、试流程，再把结果告诉用户。
`}),A={"2026-02-16":`introduction-to-memoh`,"2026-05-02":`discuss-mode`,"2026-05-15":`workspace-desktop`,"2026-09-11":`cloud-computer-for-every-agent`,"2026-09-11-en":`cloud-computer-for-every-agent`,"2026-09-15":`cloud-computer-for-every-agent`},j=e=>{let t=/^---\n([\s\S]*?)\n---\n?/.exec(e);return t?{meta:Object.fromEntries(t[1].split(`
`).map(e=>{let t=e.indexOf(`:`);if(t!==-1)return[e.slice(0,t).trim(),e.slice(t+1).trim().replace(/^["']|["']$/g,``)]}).filter(e=>!!e)),body:e.slice(t[0].length)}:{meta:{},body:e}},M=e=>e.replace(/!\[[^\]]*]\([^)]+\)/g,``).replace(/\[([^\]]+)]\([^)]+\)/g,`$1`).replace(/[`*_>#|:-]/g,``).replace(/\s+/g,` `).trim(),N=e=>{let t=M(e.split(`
`).find(e=>{let t=e.trim();return t&&!t.startsWith(`#`)&&!t.startsWith(`:::`)&&!t.startsWith(`![`)&&!t.startsWith(`- `)&&!t.startsWith(`|`)&&!t.startsWith("```")})??``);return t.length>180?`${t.slice(0,177)}...`:t},P=e=>{let t=M(e.replace(/```[\s\S]*?```/g,``)).match(/[\u4e00-\u9fff]|[A-Za-z0-9]+/g)?.length??0;return Math.max(2,Math.round(t/320))},F=e=>{let t=e.replace(/\\/g,`/`).split(`/`),n=t.pop()?.replace(/\.md$/,``),r=t.pop();if(!(!n||n===`index`)&&!(r!==`en`&&r!==`zh`))return{slug:n,locale:r}},I=(e,t)=>{let n=F(e);if(!n)return;let{meta:r,body:i}=j(t),a=i.replace(/^#\s+.+\n+/,``),o=/^(\d{4}-\d{2}-\d{2})/.exec(n.slug);return{slug:n.slug,locale:n.locale,title:r.title||n.slug,author:r.author||`Team Memoh`,dateKey:r.date||o?.[1]||n.slug,excerpt:N(a),readingMinutes:P(a),body:a}},L=new Map;for(let[e,t]of Object.entries(k)){let n=I(e,t);if(!n)continue;let r=L.get(n.slug)??{};r[n.locale]=n,L.set(n.slug,r)}var R=e=>e===`zh`?`zh`:`en`,z={zh:`zh-CN`,ja:`ja`},B=(e,t)=>{let n=L.get(e);if(n)return n[t]??n.en??n.zh},V=e=>A[e]??e,H=(e,t)=>{let n=/^(\d{4}-\d{2}-\d{2})$/.exec(e);return n?new Intl.DateTimeFormat(z[t]??`en`,{month:`short`,day:`numeric`,year:`numeric`}).format(new Date(`${n[1]}T00:00:00Z`)):e},U=e=>{let t=R(e);return[...L.keys()].map(e=>B(e,t)).filter(e=>!!e).sort((e,t)=>t.dateKey.localeCompare(e.dateKey))},W=(e,t)=>B(V(decodeURIComponent(e)),R(t)),G={class:`w-full max-w-[1080px] min-h-[calc(100vh-3.5rem)] mx-auto px-4 md:px-8 pt-[112px] md:pt-[148px] pb-[120px] relative z-10`},K={key:0,class:`flex flex-col gap-12`},q={class:`max-w-[760px] flex flex-col gap-5`},J={class:`inline-flex w-fit items-center gap-2 rounded-md border border-border bg-background px-3 py-1.5 text-xs font-medium text-muted-foreground`},Y={class:`flex flex-col gap-4`},X={class:`text-4xl md:text-6xl font-semibold tracking-tight text-foreground leading-[1.05]`},Z={class:`text-base md:text-lg text-muted-foreground leading-relaxed max-w-[680px]`},Q={class:`p-6 md:p-8 flex flex-col gap-6`},ne={class:`flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground`},re={class:`inline-flex items-center gap-1.5`},ie={class:`inline-flex items-center gap-1.5`},ae={class:`inline-flex items-center gap-1.5`},oe={class:`flex flex-col gap-3`},se={class:`text-xs font-medium uppercase tracking-[0.16em] text-primary`},ce={class:`text-2xl md:text-3xl font-semibold tracking-tight text-foreground leading-tight`},le={class:`text-sm md:text-base text-muted-foreground leading-relaxed`},ue={class:`inline-flex items-center gap-2 text-sm font-medium text-foreground`},de={class:`grid grid-cols-1 md:grid-cols-2 gap-4`},fe={class:`flex flex-col gap-4`},pe={class:`flex items-center justify-between gap-3 text-xs text-muted-foreground`},me={class:`flex flex-col gap-3`},he={class:`text-lg font-semibold tracking-tight text-foreground leading-snug`},ge={class:`text-sm text-muted-foreground leading-relaxed`},_e={class:`mt-8 inline-flex items-center gap-2 text-sm font-medium text-foreground`},ve={key:1,class:`mx-auto max-w-[820px]`},ye={class:`mb-10 flex flex-col gap-5 border-b border-border pb-8`},be={class:`flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground`},xe={class:`inline-flex items-center gap-1.5`},Se={class:`inline-flex items-center gap-1.5`},Ce={class:`inline-flex items-center gap-1.5`},we={class:`text-3xl md:text-5xl font-semibold tracking-tight text-foreground leading-tight`},Te={class:`text-base md:text-lg text-muted-foreground leading-relaxed`},Ee={key:0,class:`rounded-xl border border-border bg-background p-6 text-sm text-muted-foreground`},De=[`innerHTML`],$={key:2,class:`mt-14 border-t border-border pt-8`},Oe={class:`mb-4 text-lg font-semibold tracking-tight text-foreground`},ke={class:`grid grid-cols-1 md:grid-cols-3 gap-4`},Ae={class:`mb-2 text-xs text-muted-foreground`},je={class:`text-sm font-medium leading-snug text-foreground`},Me={key:2,class:`mx-auto flex max-w-[640px] flex-col items-center gap-5 py-24 text-center`},Ne={class:`rounded-xl border border-border bg-muted p-3 text-muted-foreground`},Pe={class:`text-3xl font-semibold tracking-tight text-foreground`},Fe={class:`text-muted-foreground`},Ie=C(t({__name:`BlogsPage`,setup(t){let g=te(),C=w(),{t:k,locale:A}=m(),j=y(()=>{let e=g.params.slug;return typeof e==`string`?e:``}),M=y(()=>j.value.length>0),N=y(()=>U(A.value)),P=y(()=>j.value?W(j.value,A.value):void 0),F=y(()=>N.value[0]),I=y(()=>N.value.filter(e=>e.slug!==P.value?.slug).slice(0,3)),L=h(``),R=h(!1),z=0,B=y(()=>P.value?k(`blog.seoPostTitle`,{title:P.value.title}):M.value?k(`blog.seoNotFoundTitle`):k(`blog.seoTitle`)),V=y(()=>P.value?.excerpt||k(`blog.seoDescription`));return d({htmlAttrs:{lang:y(()=>A.value)}}),u({title:()=>B.value,description:()=>V.value,ogTitle:()=>B.value,ogDescription:()=>V.value,ogType:`article`,ogUrl:()=>`https://memoh.ai${g.path}`,twitterCard:`summary_large_image`,twitterTitle:()=>B.value,twitterDescription:()=>V.value}),l(P,async e=>{let t=++z;if(L.value=``,!e){R.value=!1;return}R.value=!0;let{renderMarkdown:n}=await E(async()=>{let{renderMarkdown:e}=await import(`./markdown-DLfpP03F.js`);return{renderMarkdown:e}},[]),r=await n(e.body);t===z&&(L.value=r,R.value=!1)},{immediate:!0}),(t,l)=>(c(),o(`main`,G,[M.value?P.value?(c(),o(`article`,ve,[f(p(T),{to:`/blogs`,class:`mb-8 inline-flex items-center gap-2 rounded-md px-0 py-2 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2`},{default:e(()=>[f(p(ee),{class:`w-4 h-4`}),i(` `+r(t.$t(`blog.back`)),1)]),_:1}),a(`header`,ye,[a(`div`,be,[a(`span`,xe,[f(p(x),{class:`w-3.5 h-3.5`}),i(` `+r(p(H)(P.value.dateKey,p(A))),1)]),a(`span`,Se,[f(p(O),{class:`w-3.5 h-3.5`}),i(` `+r(P.value.author),1)]),a(`span`,Ce,[f(p(S),{class:`w-3.5 h-3.5`}),i(` `+r(t.$t(`blog.minRead`,{n:P.value.readingMinutes})),1)])]),a(`h1`,we,r(P.value.title),1),a(`p`,Te,r(P.value.excerpt),1)]),R.value?(c(),o(`div`,Ee,r(t.$t(`blog.rendering`)),1)):(c(),o(`div`,{key:1,class:`blog-body`,innerHTML:L.value},null,8,De)),I.value.length?(c(),o(`section`,$,[a(`h2`,Oe,r(t.$t(`blog.more`)),1),a(`div`,ke,[(c(!0),o(_,null,s(I.value,t=>(c(),n(p(T),{key:t.slug,to:`/blogs/${t.slug}`,class:`group rounded-xl border border-border bg-background p-4 transition-all hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2`},{default:e(()=>[a(`p`,Ae,r(p(H)(t.dateKey,p(A))),1),a(`h3`,je,r(t.title),1)]),_:2},1032,[`to`]))),128))])])):v(``,!0)])):(c(),o(`section`,Me,[a(`div`,Ne,[f(p(D),{class:`w-6 h-6`})]),a(`h1`,Pe,r(t.$t(`blog.notFoundTitle`)),1),a(`p`,Fe,r(t.$t(`blog.notFoundDescription`)),1),a(`button`,{type:`button`,class:`inline-flex items-center justify-center rounded-full bg-foreground px-5 py-2.5 text-sm font-medium text-background transition-colors hover:bg-foreground/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2`,onClick:l[0]||=e=>p(C).push(`/blogs`)},r(t.$t(`blog.back`)),1)])):(c(),o(`section`,K,[a(`header`,q,[a(`div`,J,[f(p(D),{class:`w-3.5 h-3.5`}),i(` `+r(t.$t(`blog.badge`)),1)]),a(`div`,Y,[a(`h1`,X,r(t.$t(`blog.indexTitle`)),1),a(`p`,Z,r(t.$t(`blog.indexDescription`)),1)])]),F.value?(c(),n(p(T),{key:0,to:`/blogs/${F.value.slug}`,class:`group grid grid-cols-1 md:grid-cols-[1.1fr_0.9fr] overflow-hidden rounded-xl border border-border bg-background transition-all duration-300 hover:-translate-y-1 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2`},{default:e(()=>[a(`div`,Q,[a(`div`,ne,[a(`span`,re,[f(p(x),{class:`w-3.5 h-3.5`}),i(` `+r(p(H)(F.value.dateKey,p(A))),1)]),a(`span`,ie,[f(p(O),{class:`w-3.5 h-3.5`}),i(` `+r(F.value.author),1)]),a(`span`,ae,[f(p(S),{class:`w-3.5 h-3.5`}),i(` `+r(t.$t(`blog.minRead`,{n:F.value.readingMinutes})),1)])]),a(`div`,oe,[a(`span`,se,r(t.$t(`blog.latest`)),1),a(`h2`,ce,r(F.value.title),1),a(`p`,le,r(F.value.excerpt),1)]),a(`span`,ue,[i(r(t.$t(`blog.readArticle`))+` `,1),f(p(b),{class:`w-4 h-4 transition-transform group-hover:translate-x-0.5`})])]),l[1]||=a(`div`,{class:`border-t md:border-t-0 md:border-l border-border bg-muted/35 p-6 md:p-8 flex items-end`},[a(`div`,{class:`w-full font-mono text-xs text-muted-foreground leading-relaxed`},[a(`div`,{class:`mb-4 flex items-center gap-1.5`},[a(`span`,{class:`h-2.5 w-2.5 rounded-full bg-red-400/80`}),a(`span`,{class:`h-2.5 w-2.5 rounded-full bg-yellow-400/80`}),a(`span`,{class:`h-2.5 w-2.5 rounded-full bg-green-400/80`})]),a(`p`,null,`agent.workspace.boot()`),a(`p`,{class:`text-foreground`},`container.isolated = true`),a(`p`,null,`memory.local_first = true`),a(`p`,null,`status = "always-on"`)])],-1)]),_:1},8,[`to`])):v(``,!0),a(`section`,de,[(c(!0),o(_,null,s(N.value.slice(1),o=>(c(),n(p(T),{key:o.slug,to:`/blogs/${o.slug}`,class:`group flex min-h-[260px] flex-col justify-between rounded-xl border border-border bg-background p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2`},{default:e(()=>[a(`div`,fe,[a(`div`,pe,[a(`span`,null,r(p(H)(o.dateKey,p(A))),1),a(`span`,null,r(t.$t(`blog.min`,{n:o.readingMinutes})),1)]),a(`div`,me,[a(`h2`,he,r(o.title),1),a(`p`,ge,r(o.excerpt),1)])]),a(`span`,_e,[i(r(t.$t(`blog.readArticle`))+` `,1),f(p(b),{class:`w-4 h-4 transition-transform group-hover:translate-x-0.5`})])]),_:2},1032,[`to`]))),128))])]))]))}}),[[`__scopeId`,`data-v-d10353af`]]);export{Ie as default};