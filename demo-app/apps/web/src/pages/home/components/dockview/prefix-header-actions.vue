...
function refreshFirstGroup() {
  const dock = props.params.containerApi
  firstGroupId.value = dock.groups.find(group =>
    !dock.adjacentGroupInDirection(group, 'left') && !dock.adjacentGroupInDirection(group, 'above'),
  )?.id ?? dock.groups[0]?.id ?? ''
}
...
const disposables = [
  props.params.containerApi.onDidLayoutChange(refreshFirstGroup),
