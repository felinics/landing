...
export const providers = ['OpenAI', 'Anthropic', 'Google'].map((name, i) => ({ id: `provider-${i}`, name, icon: name.toLowerCase(), client_type: ['openai', 'anthropic', 'google'][i], provider_template_id: name.toLowerCase(), enable: true, config: { api_key: 'demo-key', base_url: '' }, metadata: {}, created_at: now, updated_at: now }))
export const models = ['GPT-5', 'Claude Sonnet 4', 'Gemini 2.5 Pro', 'GPT-4.1', 'text-embedding-3-small', 'GPT Image 1'].map((name,i) => ({ id: `model-${i}`, name, model_id: ['gpt-5','claude-sonnet-4','gemini-2.5-pro','gpt-4.1','text-embedding-3-small','gpt-image-1'][i], provider_id: `provider-${i<3?i:0}`, type: i===4?'embedding':i===5?'image':'chat', enable: true, config: { context_window: 200000, max_tokens: 16384, vision: true, tool_call: true }, reasoning: { supported: true, efforts: ['low','medium','high'], default_effort: 'medium' } }))
export const settings = { chat_model_id: 'model-1', compaction_model_id: 'model-0', chat_runtime: 'native', default_bot_agent_id: '', reasoning_effort: 'medium', language: 'en', timezone: 'Asia/Singapore', compaction_enabled: true, compaction_threshold: 80000, compaction_target_percent: 50, search_provider_id: 'search-0', memory_provider_id: 'memory-0', fetch_provider_id: 'fetch-0', image_model_id: 'model-5', tts_model_id: 'speech-0', transcription_model_id: 'transcription-0', video_model_id: 'video-0', display_enabled: false, show_tool_calls_in_im: true, persist_full_tool_results: true, acl_default_effect: 'allow', tool_approval_config: { mode: 'auto' } }
