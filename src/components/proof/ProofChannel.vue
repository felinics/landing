...
<template>
  <div class="proof-surface h-full w-full overflow-hidden" :style="{ backgroundColor: page }">
    <!-- Memoh 原生聊天：用户右品牌气泡 + 助手左纯文本 -->
    <div v-if="platform === 'desktop'" class="flex h-full flex-col">
      <div class="flex shrink-0 items-center border-b border-white/[0.06] px-3.5 py-2.5">
        <span class="text-[11px] font-[550] tracking-[-0.02em] text-white/70">Felinic</span>
      </div>
      <div class="flex min-h-0 flex-1 flex-col justify-end gap-2.5 p-3">
        <template v-for="(m, i) in messages" :key="i">
          <div v-if="m.side === 'user'" class="flex justify-end">
            <p class="w-fit max-w-[86%] rounded-2xl bg-chat-user-bubble px-3 py-1.5 text-[11.5px] leading-snug text-chat-user-bubble-fg">{{ t(m.key) }}</p>
          </div>
          <p v-else class="text-[11.5px] leading-relaxed text-foreground/90">{{ t(m.key) }}</p>
        </template>
      </div>
    </div>
    ...
    <div v-else-if="platform === 'telegram' || platform === 'wechat'" class="flex h-full flex-col">
      <div class="flex shrink-0 flex-col justify-center px-3.5 py-2" :style="{ backgroundColor: cfg.header }">
        <p class="text-[12px] font-medium leading-tight text-white">Felinic</p>
        <p v-if="cfg.online" class="text-[10px] leading-tight text-[#6cb1e1]">{{ t('proof.channel.online') }}</p>
      </div>
      <div class="flex min-h-0 flex-1 flex-col justify-end gap-2 p-3">
        <div v-for="(m, i) in messages" :key="i" class="flex" :class="m.side === 'user' ? 'justify-end' : ''">
          <p class="max-w-[82%] px-2.5 py-1.5 text-[11.5px] leading-snug" :class="m.side === 'user' ? cfg.outBubble : cfg.inBubble">{{ t(m.key) }}</p>
        </div>
      </div>
    </div>
    ...
    <div v-else class="flex h-full flex-col">
      <div class="flex shrink-0 items-center bg-[#2b2d31] px-3 py-2.5">
        <span class="truncate text-[12px] font-semibold text-white/90">{{ t('proof.channel.discord.channel') }}</span>
      </div>
      <div class="flex min-h-0 flex-1 flex-col justify-end gap-2 p-3">
        <div v-for="(m, i) in messages" :key="i" class="min-w-0">
          <span class="text-[11px] font-medium leading-tight" :class="m.side === 'user' ? 'text-[#949ba4]' : 'text-[#c8a2ff]'">{{ m.side === 'user' ? t('proof.channel.you') : 'Felinic' }}</span>
          <p class="text-[11px] leading-snug text-[#dbdee1]">{{ t(m.key) }}</p>
        </div>
      </div>
    </div>
  </div>
