 ...
 */
function uploadWithServerProgress<T>(options: UploadOptions): Promise<T> {
...
// Browser demo: preserve the upload contract without starting an XMLHttpRequest.
export const uploadWithProgress: typeof uploadWithServerProgress = async <T>(options: UploadOptions): Promise<T> => {
  const response = await fetch(sdkApiUrl({url: options.url}), {method: 'POST', body: options.formData, signal: options.signal})
